...
import numpy as np
import torch
from models.model import SCAN, xattn_score_t2i, xattn_score_i2t
...
def shard_xattn_t2i_model(model, images, captions,cap_masks, img_masks,  opt, shard_size=128):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            cap = Variable(torch.from_numpy(captions[cap_start:cap_end])).long().cuda()
            ...
            cap = cap.unsqueeze(0).expand(n_img,n_cap,cap.size(1)).contiguous().view(-1,cap.size(1))
            ...
            scores = model.txt_enc(cap, cap_type, cap_mask, im, img_mask, istest=True)
...
def shard_xattn_i2t_model(model, images, captions, cap_masks, img_masks, opt, shard_size=128):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            scores = model.txt_enc(cap, cap_type, cap_mask,im,img_mask)
            sim = scores.view(n_img,n_cap)
            d[im_start:im_end, cap_start:cap_end] = sim.data.cpu().numpy()
